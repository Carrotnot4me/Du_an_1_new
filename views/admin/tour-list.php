...
<body>
  <div class="app">
    ...
    <aside class="sidebar" id="sidebar">
      <div class="logo">
           <img src="<?= htmlspecialchars($_SESSION['user_avatar'] ?? 'https://ui-avatars.com/api/?name=User&background=random') ?>" 
             alt="Avatar" 
             title="<?= htmlspecialchars($_SESSION['user_avatar'] ?? '') ?>"
      ...
      <nav>
        ...
        <div class="nav-item active"><i class="bi bi-airplane"></i><a href="<?= htmlspecialchars($_SERVER['PHP_SELF']) ?>?action=tour-list"> Quản lý
          Tour</a></div>
        <div class="nav-item"><i class="bi bi-calendar-check"></i><a href=""> Đợt khởi hành</a></div>
        <div class="nav-item"><i class="bi bi-bookmark-check"></i><a href="<?= htmlspecialchars($_SERVER['PHP_SELF']) ?>?action=booking"> Đặt chỗ</a></div>
        <div class="nav-item"><i class="bi bi-people-fill"></i><a href="/views/admin/customer-list.html"> Khách hàng</a>
        </div>
        <div class="nav-item"><i class="bi bi-person-badge"></i><a href="/views/admin/guide-list.html">Hướng dẫn
            viên</a></div>
        <div class="nav-item"><i class="bi bi-shield-lock-fill"></i><a href="#">Bảo mật</a></div>
        <div class="nav-item"><i class="bi bi-gear-fill"></i><a href="#">Cài đặt nâng cao</a></div>

      </nav>

      <div style="margin-top:auto; font-size:13px; opacity:.9">
        <div>Người dùng: <strong><?= htmlspecialchars($_SESSION['user_role'] ?? 'User') ?></strong></div>
        <div style="margin-top:6px">Email: <small><?= htmlspecialchars($_SESSION['user_email'] ?? '') ?></small></div>
      </div>
    </aside>
