...
// Bắt buộc phải có 2 dòng này đầu tiên
require_once __DIR__ . '/commons/env.php';
require_once __DIR__ . '/commons/function.php';

// Lấy controller và action từ URL
$controller = $_GET['controller'] ?? 'booking';  // mặc định là booking
$action     = $_GET['action']      ?? 'add';      // mặc định là trang thêm booking

// Chuẩn hóa tên controller (viết hoa chữ cái đầu)
...
if (!file_exists($controllerFile)) {
    die("Không tìm thấy controller: {$controller}.php");
}
...
// Kiểm tra action (method) có tồn tại không
if (!method_exists($controllerObject, $action)) {
    die("Action {$action} không tồn tại trong {$controller}!");
}

// Gợi ý: add, list, save, delete");
